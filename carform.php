<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if(mysqli_num_rows(mysqli_query($conn, "SELECT * FROM buy_car WHERE csku='{$csku}' OR car_name='{$carname}' AND car_model='{$carmodel}' "))> 0){
            $msg="<div class='alert alert-danger'>Car Already Exists</div>";}
        else{
            $sql="INSERT INTO buy_car (csku,car_name,car_model,car_type,engine_power,transmission,color,passenger_capacity,drive,price,car_img,mileage,fuel_type,Car_description,quantity) VALUES('{$csku}','{$carname}','{$carmodel}','{$cartype}','{$engine}','{$transmission}','{$color}','{$passcap}','{$drivewheels}','{$price}','{$img}','{$mileage}','{$fuel}','{$description}','{$quantity}') ";
            ...
            <div class="workinghny-form-grid">
                <div class="main-mockup">
                    ...
                    <div class="content-wthree">
                        ...
                        <p>Insert car details for sell</p>
                        <?php echo $msg; ?>
                        <form action="carform.php" method="post" enctype="multipart/form-data" > 
                            <input type="text" class="csku" name="csku" placeholder="csku" value="<?php if (isset($_POST['submit'])) { echo $csku; } ?>" required>
                            ...
                            <input type="text" class="fuel_type" name="fuel_type" placeholder="Fuel Type" value="<?php if (isset($_POST['submit'])) { echo $fuel; } ?>" required>
                            ...
                            <input type="text" class="Car_description" name="Car_description" placeholder="Description" value="<?php if (isset($_POST['submit'])) { echo $description; } ?>" required>
